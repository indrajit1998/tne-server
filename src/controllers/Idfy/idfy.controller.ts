...
async function createKycTaskRecord(params: {
  ...
  const { userId, groupId, taskType, taskId, requestId, initialStatus = 'pending', raw } = params;
 ...
 * }
 */
export const startDocumentVerification = async (req: AuthRequest, res: Response) => {
  ...
  if (!document1 || typeof document1 !== 'string') {
    return res.status(400).json({ message: 'document1 (image base64 or URL) is required' });
  }

  const normalizedDoc1 = normalizeBase64(document1);
  const normalizedDoc2 = document2 ? normalizeBase64(document2) : undefined;

  // Basic validation
  if (!type || !['ind_pan', 'ind_aadhaar', 'ind_driving_license'].includes(type)) {
    return res.status(400).json({ message: 'Invalid or missing `type`' });
  }

  if (!normalizedDoc1) {
    return res.status(400).json({ message: 'document1 normalization failed' });
  }

  // Aadhaar requires consent per docs
  if (type === 'ind_aadhaar') {
    const consent = extra?.consent || req.body?.consent;
    if (!consent || typeof consent !== 'string' || !['yes', 'no'].includes(consent)) {
      return res.status(400).json({ message: "Aadhaar requires `consent: 'yes'|'no'` in `extra`" });
    }
    // also document2 (back) is required for aadhaar in docs
    if (!document2) {
      return res.status(400).json({ message: 'Aadhaar requires document2 (back image) as well' });
    }
  }

  // Driving license expects front+back
  if (type === 'ind_driving_license' && !document2) {
    return res.status(400).json({ message: 'Driving license requires document2 (back image)' });
  }
  ...
  try {
    ...
    const opts = {
      document1: normalizedDoc1,
      document2: normalizedDoc2,
      groupId,
      extra,
    };
 ...
 */
export const startFaceVerification = async (req: AuthRequest, res: Response) => {
  const userId = req.user;
  ...
  if (!image || typeof image !== 'string') {
    return res.status(400).json({ message: 'Image (base64 or URL) is required' });
  }

  const normalizedImage = normalizeBase64(image);

  if (!normalizedImage) {
    return res.status(400).json({ message: 'Image normalization failed' });
  }
  ...
  try {
    const {
      ...
      requestId,
      raw,
    } = await createFaceLivenessTask(normalizedImage, groupId, options);
    ...
    logger.info({ userId, taskId, requestId, groupId: gid }, 'Face verification started');
    ...
    await session.abortTransaction();
    logger.error({ err, body: req.body, userId }, 'startFaceVerification failed');
    return res.status(500).json({
...
export const idfyWebhook = async (req: AuthRequest, res: Response) => {
  ...
  const isLiveFromResult = (r: any) =>
    !!(r?.is_live || r?.result?.is_live || r?.output?.is_live || r?.face?.is_live);
  ...
  try {
    ...
    let task = await KycTask.findOne({ requestId: request_id }).session(session);
    ...
    if (!task) {
      ...
      logger.warn({ request_id, group_id }, '[WEBHOOK] ⚠️ KycTask not found - acknowledged');
      await session.commitTransaction();
    ...
    if (task.status === 'completed' && ['completed', 'success'].includes(finalStatus)) {
      logger.info({ request_id }, '[WEBHOOK] Duplicate verified webhook ignored');
      await session.commitTransaction();
    ...
    logger.info({ taskId: task._id, taskType: task.taskType }, '[WEBHOOK] ✅ KycTask updated');
    ...
    if (!['face', 'ind_pan', 'ind_aadhaar', 'ind_driving_license'].includes(kycField)) {
      logger.warn({ kycField }, '[WEBHOOK] ⚠️ Unknown kycField');
      // commit and ack
      await session.commitTransaction();
      return res.status(200).json({ message: 'Unknown kyc type, acknowledged' });
    }

    // Determine the new subdoc status (handle face liveness)
    let newSubdocStatus: 'pending' | 'verified' | 'failed' | 'not_provided' = 'pending';
    ...
    const user = await User.findOne({ 'kyc.groupId': group_id }).session(session);
    if (!user) {
    ...
    if (existing && existing.status === 'verified' && newSubdocStatus !== 'verified') {
      logger.info(
    ...
    if ((panVerified || aadhaarVerified) && faceVerified && (!dlProvided || dlVerified)) {
      user.kyc.overallStatus = 'verified';
      user.isKYCVerified = true;
    } else if (
      [
